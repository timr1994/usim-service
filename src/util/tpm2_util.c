...
TSS2_RC tpm2_create_primary_key_rsa2048(
    ...
    TPM2B_AUTH authValueSH = {.size = 0, .buffer =
        {0}};
    ...
    TPM2B_AUTH authValuePK = {.size = 0, .buffer =
        {0}};
    ...
    if (r != TSS2_RC_SUCCESS) {
        ...
        fprintf(stdout, "%s", "Primary Key created successfully.");
    }
...
TSS2_RC tpm2_store_key_in_nvram(ESYS_CONTEXT* ctx, const ESYS_TR* key_handle) {
    ...
    if (r != TSS2_RC_SUCCESS) {
        ...
        fprintf(stdout, "%s", "Primary Key successfully stored in NVRAM.");
    }
...
uint32_t tpm2_get_boot_count(ESYS_CONTEXT* ctx) {
    TPM2B_ATTEST *timeInfo = malloc(sizeof (TPM2B_ATTEST));
    TPMT_SIGNATURE *sig = malloc(sizeof (TPMT_SIGNATURE));
    Esys_GetTime_Finish(ctx, &timeInfo, &sig);
    TPMS_ATTEST *att = (TPMS_ATTEST *) timeInfo->attestationData;
    return att->clockInfo.restartCount;
}
